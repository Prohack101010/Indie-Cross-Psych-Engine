...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		['Sans Bones Attack', "Funny bones for you to dodge ;).\nValue 1: 'blue', 'orange' or 'normal'.\nBlue is blue bones, orange is orange... blah blah\nValue 2: Not Needed."],
		[
			'Dadbattle Spotlight',
			"Used in Dad Battle,\nValue 1: 0/1 = ON/OFF,\n2 = Target Dad\n3 = Target BF"
		],
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if mobileC || virtualPad.buttonUp.pressed || virtualPad.buttonDown.pressed #end)
			{
				...
				if (FlxG.keys.pressed.CONTROL)
					holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT #if mobileC || virtualPad.buttonY.pressed #end)
					holdingShift = 4;
				...
				if (note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1)
				{
					...
					if (!playedSound[data])
					{
							...
							&& ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)))
						{
							...
							if (note.hitsoundDirectory == null || note.hitsoundDirectory == '')
								FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < 4 ? -0.3 : 0.3; // would be coolio
							else
								FlxG.sound.play(Paths.sound(soundToPlay, note.hitsoundDirectory)).pan = note.noteData < 4 ? -0.3 : 0.3; // would be coolio
	...
	{
		var healthIconP1:String = loadDataFromCharacter(_song.player1, 'icon');
		var healthIconP2:String = loadDataFromCharacter(_song.player2, 'icon');
		var animatedP1:Bool = false;
		if(loadDataFromCharacter(_song.player1, 'isAnimated') == 'true')
		animatedP1 = true;
		var animatedP2:Bool = false;
		if(loadDataFromCharacter(_song.player2, 'isAnimated') == 'true')
		animatedP2 = true;

		if (_song.notes[curSec].mustHitSection)
		{
			leftIcon.changeIcon(healthIconP1, animatedP1);
			rightIcon.changeIcon(healthIconP2, animatedP2);
			if (_song.notes[curSec].gfSection)
				leftIcon.changeIcon('gf');
		}
		else
		{
			leftIcon.changeIcon(healthIconP2, animatedP2);
			rightIcon.changeIcon(healthIconP1, animatedP1);
			if (_song.notes[curSec].gfSection)
				leftIcon.changeIcon('gf');
		}
	}

	function loadDataFromCharacter(char:String, ?data:String = 'icon')
	{
		...
		#end
		var daReturn:String = 'null';
		var json:CharacterFile = cast Json.parse(rawJson);
		if(data == 'icon')
			daReturn = json.healthicon;
		else if(data == 'isAnimated'){
			if(json.animatedHealthIcon)
				daReturn = 'true';
			else
				daReturn = 'false';
		}

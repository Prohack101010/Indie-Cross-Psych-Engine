...
class MainMenuState extends MusicBeatState
{
	...
	var optionShit:Array<String> = ['storyMode', 'freeplay', 'options', 'credits', 'awards'];
	...
	override function create()
	{
		...
		storyMode = new FlxSprite(-100, -435).loadGraphic(Paths.image('mainmenu/opened/Story_mode'));
		...
		storyModeSplash = new FlxSprite(-100, -435).loadGraphic(Paths.image('mainmenu/opened/Story_mode flash'));
		storyModeSplash.scrollFactor.set();
		storyModeSplash.antialiasing = ClientPrefs.globalAntialiasing;
		storyModeSplash.setGraphicSize(Std.int(storyModeSplash.width * 0.7));
		storyModeSplash.x -= 200;
		storyModeSplash.y += 230;
		storyModeSplash.alpha = 0;
		add(storyModeSplash);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				if (curSelected == 0)
				{
					FlxTween.tween(storyModeSplash, {alpha: 1}, 0.1, {ease: FlxEase.linear, onComplete: function(twn:FlxTween) { FlxTween.tween(storyModeSplash, {alpha: 0}, 0.4, {ease: FlxEase.linear, onComplete: function(twn:FlxTween) { goToState(); }}); }});
				}

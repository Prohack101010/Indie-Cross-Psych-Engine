...
class NightmareWeek {
	public static var weeksLoaded:Map<String, NightmareWeek> = new Map<String, NightmareWeek>();
	...
	// HELP: Is there any way to convert a WeekFile to NightmareWeek without having to put all variables there manually? I'm kind of a noob in haxe lmao
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'nightmare/' + sexList[i] + '.json';
				if(!weeksLoaded.exists(sexList[i])) {
					var week:WeekFile = getWeekFile(fileToCheck);
					if(week != null) {
						var weekFile:NightmareWeek = new NightmareWeek(week, sexList[i]);
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + 'nightmare/';
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int)
	{
		if(!weeksLoaded.exists(weekToCheck))
		{
			...
			if(week != null)
			{
				var weekFile:NightmareWeek = new NightmareWeek(week, weekToCheck);
				if(i >= originalLength)
				{
					#if MODS_ALLOWED
	...
	public static function getCurrentWeek():NightmareWeek {
		return weeksLoaded.get(weeksList[PlayState.storyWeek]);
	}

	public static function setDirectoryFromWeek(?data:NightmareWeek = null) {

...
class KadeAlphabet extends FlxSpriteGroup
{
	...
	public var lettersArray:Array<KadeAlphaCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:KadeAlphaCharacter = new KadeAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:KadeAlphaCharacter = new KadeAlphaCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null)
	{
		...
		if (loopNum <= splitWords.length && splitWords[loopNum] != null)
		{
			...
			var isAlphabet:Bool = KadeAlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:KadeAlphaCharacter = new KadeAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:KadeAlphaCharacter = new KadeAlphaCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class KadeAlphaCharacter extends FlxSprite
{
	...
	{
		super(x, y);
		var tex = Paths.getAlphabetAtlas('kadeAlphabet');
		frames = tex;

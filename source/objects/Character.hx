...
class Character extends FlxSprite
{
	...
	public var animatedHealthIcon:Bool = false;
	...
	public var loopedAnimations:Array<String> = [];
	...
	public function new(x:Float, y:Float, ?character:String = 'bf', ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			default:
				...
				#end
				{
					path = #if MODS_ALLOWED SUtil.getPath() + #end
				...
				animatedHealthIcon = json.animatedHealthIcon;
				...
				if (animationsArray != null && animationsArray.length > 0)
				{
					for (anim in animationsArray)
					{
						...
						if (animLoop)
							loopedAnimations.push(animAnim);
	...
	override function update(elapsed:Float)
	{
		if (!debugMode && animation.curAnim != null)
		{
			...
			else if (specialAnim && animation.curAnim.finished && !loopedAnimations.contains(animation.curAnim.name))
			{
				specialAnim = false;
				dance();
			}
	...
	@:noCompletion
	private function set_specialAnim(value:Bool):Bool
	{
		specialAnim = value;
		if (value == false)
			dance();

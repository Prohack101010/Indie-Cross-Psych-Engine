...
class FreeplayNightmareState extends MusicBeatState
{
	...
	override function create()
	{
		...
		NightmareWeek.reloadWeekFiles(false);
		...
		for (i in 0...NightmareWeek.weeksList.length) {
			if(weekIsLocked(NightmareWeek.weeksList[i])) continue;

			var leWeek:NightmareWeek = NightmareWeek.weeksLoaded.get(NightmareWeek.weeksList[i]);
			...
			NightmareWeek.setDirectoryFromWeek(leWeek);
		...
		NightmareWeek.loadTheFirstEnabledMod();
		...
		NightmareWeek.setDirectoryFromWeek();
	...
	function weekIsLocked(name:String):Bool {
		var leWeek:NightmareWeek = NightmareWeek.weeksLoaded.get(name);
	...
	override function update(elapsed:Float)
	{
		...
		else if (accepted)
		{
			...
			trace('CURRENT WEEK: ' + NightmareWeek.getWeekFileName());
			if(colorTween != null) {
				colorTween.cancel();
			}
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		var diffStr:String = NightmareWeek.getCurrentWeek().difficulties;

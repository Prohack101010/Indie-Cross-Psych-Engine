...
class GameOverCuphead extends MusicBeatSubstate
{
	...
	public function new(time:Float = 0, totalTime:Float = 0)
	{
		...
		var bfGhost:FlxSprite = new FlxSprite(PlayState.instance.boyfriend.x, PlayState.instance.boyfriend.y);
		...
		add(bfGhost);
		FlxTween.tween(bfGhost, {y: PlayState.instance.boyfriend.y - 1500}, 4.25);
			...
			if (!isEnding)
			{
				...
				switch (type)
				{
					...
					case 1:
						...
						if((FlxG.save.data.instPrev == false && !PlayState.isStoryMode) || PlayState.isStoryMode)

...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		levelInfo.setFormat(CoolUtil.returnHudFont(levelInfo), 32);
		levelInfo.updateHitbox();
		add(levelInfo);

		var levelDifficulty:FlxText = new FlxText(20, 15 + 32, 0, Difficulty.getString().toUpperCase(), 32);
		levelDifficulty.scrollFactor.set();
		levelDifficulty.setFormat(CoolUtil.returnHudFont(levelDifficulty), 32);
		levelDifficulty.updateHitbox();
		add(levelDifficulty);

		var blueballedTxt:FlxText = new FlxText(20, 15 + 64, 0, "Blueballed: " + PlayState.deathCounter, 32);
		blueballedTxt.scrollFactor.set();
		blueballedTxt.setFormat(CoolUtil.returnHudFont(blueballedTxt), 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 101, 0, "PRACTICE MODE", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(CoolUtil.returnHudFont(practiceText), 32);
		...
		chartingText.setFormat(CoolUtil.returnHudFont(chartingText), 32);
		...
		missingText.setFormat(CoolUtil.returnHudFont(missingText), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (accepted && (cantUnpause <= 0 || !controls.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case "Exit to menu":
					...
					MusicBeatState.instance.setChrome(0);
	...
	function regenMenu():Void {
		...
		for (i in 0...menuItems.length) {
			...
			if(menuItems[i] == 'Skip Time')
			{
				...
				skipTimeText.setFormat(CoolUtil.returnHudFont(skipTimeText), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
